 ...
 * @tparam T
 */
template <class T> Lista<T>::Lista() { inicio = nullptr; } /**puntero que apunta al vacio, para inicializar la lista de manera predeterminada**/
...
void Lista<T>::invertir() {
    ...
    Nodo<T> *auxNodo =inicio, *posSiguiente, *posAnterior=nullptr;

    if(esVacia()) throw 404;

    while(auxNodo!= nullptr){

        posSiguiente =auxNodo->getSiguiente();
        auxNodo->setSiguiente(posAnterior);
        posAnterior=auxNodo;
        auxNodo=posSiguiente;
    }
